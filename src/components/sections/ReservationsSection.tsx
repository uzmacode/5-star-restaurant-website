...
export function ReservationsSection() {
  ...
  useEffect(() => { const table = searchParams.get("table"); const time = searchParams.get("time"); const occasion = searchParams.get("occasion"); const courses = searchParams.get("courses"); const guests = Number(searchParams.get("guests")); const seatingArea = table?.startsWith("Glasshouse") ? "Botanical Glasshouse" : table?.startsWith("Salon") ? "Main Dining Salon" : table?.startsWith("Hearth") ? "Chef’s Hearth Counter" : table?.startsWith("Terrace") ? "Herb Garden Terrace" : undefined; if (table || time || occasion || guests || courses) setForm((previous) => ({ ...previous, ...(table ? { table } : {}), ...(time ? { time } : {}), ...(occasion ? { occasion } : {}), ...(seatingArea ? { seatingArea } : {}), ...(courses ? { specialRequests: `${courses}-course Evening Fitting Studio menu` } : {}), ...(guests ? { guests: Math.min(8, Math.max(2, guests)) } : {}) })); }, [searchParams]);
