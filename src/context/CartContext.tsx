...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const createOrder = (data: Omit<OrderDetails, "id" | "orderNumber" | "createdAt" | "status">): OrderDetails => { void fetch("/api/orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: data.guest.name, email: data.guest.email, items: data.items, type: data.type }) }); const order: OrderDetails = { ...data, id: `ord-${Date.now()}`, orderNumber: `GT-${Math.floor(1000 + Math.random() * 9000)}`, createdAt: new Date().toISOString(), status: "Received" }; const next = [order, ...orders]; setOrders(next); localStorage.setItem(keys.orders, JSON.stringify(next)); localStorage.setItem(keys.last, JSON.stringify(order)); clearCart(); return order; };
